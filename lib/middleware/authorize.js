...
var authorize = function (req, res, next) {
  ...
  if (req.headers.authorization) {
    ...
    if (parts.length < 2) {
      ...
      return next(new OAuthError('invalid_client',
  ...
  Token.findOne({
    ...
    if (!token) {
      ...
      return next(new OAuthError('invalid_token', 'Access Token invalid'));
    }

    if (token.consumed) {
      // the token got consumed already - cancel
      return next(new OAuthError('invalid_token', 'Access Token expired'));
    }
